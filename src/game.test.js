...
function testStillLife(inputBoard) {
	...
	const allGenerationsAreTheSame =
		(JSON.stringify(inputBoard) === JSON.stringify(generationTwo)) ===
		JSON.stringify(generationThree);
	if (!allGenerationsAreTheSame) {
		throw new Error(
			'FAIL: A still life should not change across multiple generations'
		);
	}

	console.log(
		'PASS: A still life should not change across multiple generations'
	);
}
